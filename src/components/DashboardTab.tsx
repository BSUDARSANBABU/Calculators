...
export default function DashboardTab() {
  ...
  const [activeTab, setActiveTab] = useState<'time-tools' | 'sdt-push'>('time-tools');
  ...
  const [sdtTrailerAttached, setSdtTrailerAttached] = useState<boolean>(true);
  const [sdtCptChanged, setSdtCptChanged] = useState<boolean>(false);
  const [sdtTourOrRun, setSdtTourOrRun] = useState<boolean>(false);
  const [sdtIs3P, setSdtIs3P] = useState<boolean>(false);
  const [sdtCopySuccess, setSdtCopySuccess] = useState<boolean>(false);

  // Loading Compliance Calculator States
  const [originSite, setOriginSite] = useState('PBI3');
  const [originSiteWarning, setOriginSiteWarning] = useState<string | null>(null);
  ...
  const [calculatedCompliance, setCalculatedCompliance] = useState<string | null>('20-05-2026 22:31');
  ...
  const [transitHrsInput, setTransitHrsInput] = useState<string>('154');
  ...
  const handleOriginSitePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
  ...
  const handleSiteTimePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
  ...
  const handleHoursPaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    const pastedText = e.clipboardData.getData('text');
  ...
  const loadSdtPreset = (preset: typeof SDT_PRESETS[0]) => {
